...
def index():
    ...
    list = query.order_by(Book.book_id).offset(offset).limit(app.config['PAGE_SIZE']).all()
...
def cat():
    ...
    list = query.order_by(BookCat.weight, BookCat.id).all()
    resp_data['list'] = list
    resp_data['search_con'] = req
    resp_data['status_mapping'] = app.config['STATUS_MAPPING']
...
@route_book.route("/cat-set", methods=["GET", "POST"])
def catSet():
    if request.method == "GET":
        resp_data = {}
        req = request.args
...
def catOps():
    ...
    if act == "remove":
        ...
        book_cat_info.update_time = getCurrentDate()
