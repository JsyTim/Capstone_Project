...
def book():
    ...
    date_from = req['date_from'] if 'date_from' in req else default_date_from
    date_to = req['date_to'] if 'date_to' in req else default_date_to
    query = StatDailyBook.query.filter(StatDailyBook.date >= date_from) \
        .filter(StatDailyBook.date <= date_to)
    ...
    resp_data['pages'] = pages
    resp_data['current'] = 'book'
    resp_data['search_con'] = {
        'date_from': date_from,
        'date_to': date_to
    }
    return ops_render("stat/book.html", resp_data)
