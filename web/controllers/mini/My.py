...
@route_mini.route("/my/order/info")
def myOrderInfo():
    resp = {'code': 200, 'msg': '操作成功~', 'data': {}}
    member_info = g.member_info
    req = request.values
...
@route_mini.route("/my/comment/add", methods=["POST"])
def myCommentAdd():
    resp = {'code': 200, 'msg': '操作成功~', 'data': {}}
    member_info = g.member_info
    req = request.values
    ...
    pay_order_items = PayOrderItem.query.filter_by(pay_order_id=pay_order_info.id).all()
    book_ids = selectFilterObj(pay_order_items, "book_id")
    tmp_book_ids_str = '_'.join(str(s) for s in book_ids if s not in [None])
    model_comment = MemberComments()
    model_comment.book_ids = "_%s_"%tmp_book_ids_str
...
@route_mini.route("/my/comment/list")
def myCommentList():
    resp = {'code': 200, 'msg': '操作成功~', 'data': {}}
    member_info = g.member_info
