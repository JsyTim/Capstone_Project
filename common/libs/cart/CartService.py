...
class CartService():
    ...
    def deleteItem(member_id=0, items=None):
        if member_id < 1 or not items:
            return False
        for item in items:
            MemberCart.query.filter_by(book_id=item['id'], member_id=member_id).delete()
        db.session.commit()
        return True

    @staticmethod
    def setItems(member_id=0, book_id=0, number=0):
        if member_id < 1 or book_id < 1 or number < 1:
            return False
        cart_info = MemberCart.query.filter_by(book_id=book_id, member_id=member_id).first()
        if cart_info:
